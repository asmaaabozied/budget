...
class MenuHelper
{
    ...
    public static function disabledMenus($model)
    {
        if (Str::contains($model, 'SideMenuItem')) {
            return (count(self::menuDisabled())) ? self::menuDisabled() : [];
        } elseif (Str::contains($model, 'SideMenuSection')) {
            return (count(self::disabledSection())) ? self::disabledSection() : [];
        }
    }
    ...
    {
        if (Str::contains($model, 'SideMenuItem')) {
            return (count(self::menuActive())) ? self::menuActive() : [];
        } elseif (Str::contains($model, 'SideMenuSection')) {
            return (count(self::activeSection())) ? self::activeSection() : [];
        }
    }
